...
// reset localization
//pos
var ResetPos1 = new ROSLIB.Topic({
    ros: ros,
    name: '/Wait_Minda',
    messageType: 'geometry_msgs/Pose2D'
});
var ResetPos2 = new ROSLIB.Topic({
    ros: ros2,
    name: '/Wait_Minda',
    messageType: 'geometry_msgs/Pose2D'
});
var ResetPos3 = new ROSLIB.Topic({
    ros: ros3,
    name: '/Wait_Minda',
    messageType: 'geometry_msgs/Pose2D'
});
//cmd
var ResetCmd1 = new ROSLIB.Topic({
    ros: ros,
    name: '/Wait_Minda2',
    messageType: 'std_msgs/Int32'
});
var ResetCmd2 = new ROSLIB.Topic({
    ros: ros2,
    name: '/Wait_Minda2',
    messageType: 'std_msgs/Int32'
});
var ResetCmd3 = new ROSLIB.Topic({
    ros: ros3,
    name: '/Wait_Minda2',
    messageType: 'std_msgs/Int32'
});
function Reset_Localization(X,Y){
    POS = new ROSLIB.Message({
        x: X,
        y: Y,
        theta: 0
    });
    ...
    if (ChooseRobot == 1) {
        ResetPos1.publish(POS);
        setTimeout(Reset_Command,500);
    } else if (ChooseRobot == 2) {
        ResetPos2.publish(POS);
        setTimeout(Reset_Command,500);
    } else if (ChooseRobot == 3) {
        ResetPos3.publish(POS);
        setTimeout(Reset_Command,500);
    }
    //console.log("Robot : "+ChooseRobot+" Pose :"+X);
}
...
function Reset_Command(){
    ...
    if (ChooseRobot == 1) {
        ResetCmd1.publish(CMD);
    } else if (ChooseRobot == 2) {
        ResetCmd2.publish(CMD);
    } else if (ChooseRobot == 3) {
        ResetCmd3.publish(CMD);
    }
    //console.log("Robot : "+ChooseRobot+" CMD :"+CMD.data);
} 