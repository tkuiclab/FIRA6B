...
void FIRA_pathplan_class::strategy_Goalkeeper_init(int r_number){
    ...
    double hor_x = -opgoal_dis * sin(opgoal_edge_angle1*deg2rad);
    double hor_y = opgoal_dis * cos(opgoal_edge_angle1*deg2rad);
    Vector2d vectorbr(hor_x,hor_y);

    if(opgoal_edge_angle1 + opgoal_edge_angle2 - opgoal_angle < 10){
        vectorbr(0) = 0;
        if(opgoal_dis > 0.75){
            vectorbr(1) = opgoal_dis*cos(opgoal_angle * deg2rad);
        }else{
            vectorbr(1) =0;
        }
...
//    printf("opgoal_middle_angle = %f\n",opgoal_middle_angle);
    ...
    int robot_on_R = (fabs(opgoal_edge_angle1) < opgoal_edge_angle2)? 1: 0;
    
    if(ball_angle > opgoal_middle_angle){
        x = -(2/ball_dis) * sin(opgoal_edge_angle2*deg2rad);
        y = (2/ball_dis) * cos(opgoal_edge_angle2*deg2rad);
        printf("robot go left\n");
        rotAngle = -rotAngle;
        if(robot_on_R == 0 && opgoal_middle_angle +30 > ball_angle && opgoal_middle_angle < ball_angle ){
            x =0;
            y =0;
        }else if(robot_on_R == 0 && opgoal_dis < 0.4){
            x = 0;
            y = 0;
        }else if(opgoal_middle_angle-30 < ball_angle && opgoal_middle_angle > ball_angle){
            x =0;
            y =0;
        }
    }else{
        x = - (2/ball_dis) * sin(opgoal_edge_angle1*deg2rad);
        y = (2/ball_dis) * cos(opgoal_edge_angle1*deg2rad);
        printf("robot go right\n");
        if(robot_on_R == 1 && opgoal_middle_angle-30 < ball_angle && opgoal_middle_angle > ball_angle){
            x =0;
            y =0;
        }else if(robot_on_R == 1 && opgoal_dis < 0.4){
            x = 0;
            y = 0;
        }else if(opgoal_middle_angle+30 < ball_angle && opgoal_middle_angle > ball_angle){
            x =0;
...
//    double opgoal_y = opgoal_dis*cos(opgoal_angle*deg2rad);
...
//    if( fabs(ball_angle) <10 && fabs(opgoal_angle) >170 ){
//        vectorbr(0) = 0;
//        vectorbr(1) = 0;
//    }
//    env.home[r_number].v_x =vectorbr(0);
...
void FIRA_pathplan_class::strategy_Goalkeeper_catching(int r_number){
    ...
    double opgoal_angle = env.home[r_number].op_goal.distance;
    double ball_dis = env.home[r_number].ball.distance;
    ...
    double ball_x = -(2/c_ball_dis)*sin(c_ball_angle*deg2rad);
    double ball_y = (2/c_ball_dis)*cos(c_ball_angle*deg2rad);
